...
ContactSolver::ContactSolver(ContactSolverDef* def)
{
	m_step = def->step;
	m_count = def->count;
	m_positionConstraints = std::vector<ContactPositionConstraint*>(m_count);
	m_velocityConstraints = std::vector<ContactVelocityConstraint*>(m_count);
		...
		else
		{
			...
			for (;;)
			{
				...
				if (x.x >= 0.0f && x.y >= 0.0f)
				{
					...
					b2Assert(Abs(vn1 - cp1->velocityBias) < k_errorTol);
					b2Assert(Abs(vn2 - cp2->velocityBias) < k_errorTol);
#endif
					break;
				}
...
bool ContactSolver::SolvePositionConstraints()
{
	...
	for (int i = 0; i < m_count; ++i)
	{
		...
		for (int j = 0; j < pointCount; ++j)
		{
			...
			float C = Clamp(b2_baumgarte * (separation + b2_linearSlop), -b2_maxLinearCorrection, 0.0f);
...
bool ContactSolver::SolveTOIPositionConstraints(int toiIndexA, int toiIndexB)
{
	...
	for (int i = 0; i < m_count; ++i)
	{
		...
		for (int j = 0; j < pointCount; ++j)
		{
			...
			// Prevent large corrections and allow slop.
			float C = Clamp(b2_toiBaumgarte * (separation + b2_linearSlop), -b2_maxLinearCorrection, 0.0f);
